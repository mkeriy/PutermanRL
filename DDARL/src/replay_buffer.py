...
class Replay_buffer(object):
    ...
    def push(self, data):
        ...
        if self.dt is None:
            ...
            self.dt = np.dtype(
                [
                    ("state", np.float64, s_shape),
                    ("next_state", np.float64, s_shape),
                    ("action", np.float32, a_shape),
                    ("reward", np.float64, (1,)),
                    ("done", np.bool_, (1,)),
                ]
            )
    ...
    def sample_r_sorted(self, batch_size, elem=default_elem):
        if self.top_perc == 1.0:
            return self.sample(batch_size, elem)
        if not self.is_sorted:
            self.sort_r()
        cur_sorted_inds = self.sortedinds[
            : max(batch_size, int(self.size * (1 - self.top_perc)))
        ]
        return self.sample_by_inds(cur_sorted_inds, batch_size, elem)

    def sample_last(self, batch_size, last=None, elem=default_elem):
        return self.sample_by_inds(
            np.arange(max(-self.size, self.ptr - last), self.ptr), batch_size, elem
        )

    def sort_r(self):
        self.sortedinds = np.argpartition(
            self.storage[: self.size]["reward"].flatten(),
            int(self.size * (1 - self.top_perc)),
        )[::-1]
        self.is_sorted = True
